 ...
 */
uint32_t spi_transfer(BM_SPI *device,
                      uint32_t value) {
...
/**
 * @brief      Manually selects a SPI device for transfer
 *
 * @param[in]  pin   The GPIO pin and the GPIO port
 *
 * @return     The result of the operation - if invalid GPIO pin, returns false
 */
BM_SPI_RESULT spi_select(BM_GPIO_PORTPIN portpin) {

    if (gpio_write(portpin, GPIO_LOW) == GPIO_SUCCESS) {
        return SPI_SIMPLE_SUCCESS;
    }
    else {
...
/**
 * @brief      Manually deselects a SPI device
 *
 * @param[in]  pin   The GPIO pin and the GPIO port
 *
 * @return      The result of the operation - if invalid GPIO pin, returns false
 */
BM_SPI_RESULT spi_deselect(BM_GPIO_PORTPIN portpin) {

    if (gpio_write(portpin, GPIO_HIGH) == GPIO_SUCCESS) {
        return SPI_SIMPLE_SUCCESS;
    }
    else {
        return SPI_SIMPLE_ERROR;
